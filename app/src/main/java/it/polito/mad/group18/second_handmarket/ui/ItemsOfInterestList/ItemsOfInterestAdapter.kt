...
class ItemsOfInterestAdapter(var adv : MutableList<Item>, val ctx : Fragment, val recView : RecyclerView) : RecyclerView.Adapter<ItemsOfInterestAdapter.ItemsOfInterestListVH>(){

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemsOfInterestListVH {
        val v = LayoutInflater.from(parent.context)
            .inflate(R.layout.home_cards_list, parent, false)

        return ItemsOfInterestListVH(v)
    }

    override fun getItemCount(): Int = adv.size
    override fun onBindViewHolder(holder: ItemsOfInterestListVH, position: Int) {
    ...
    fun itemsSet(newItems: MutableList<Item>) {
        if (newItems != adv) {
            val diff = DiffUtil.calculateDiff(ItemsOfInterestDiffCallBack(adv, newItems))
            adv = newItems
    ...
    class ItemsOfInterestListVH(v: View) : RecyclerView.ViewHolder(v){
        ...
        fun bind(h: Item, ctx: Fragment) {
            ...
            card.setOnClickListener {
                n.navigate(
                    R.id.action_itemsOfInterestListFragment_to_itemDetailsFragment,
                    bundle
                )
            }
